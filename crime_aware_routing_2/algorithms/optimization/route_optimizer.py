...
class RouteOptimizer:
    ...
    def update_config(self, new_config: RoutingConfig) -> None:
        ...
        # Update KDE bandwidth if changed (only for KDE weighter)
        if (self.crime_weighter and hasattr(self.crime_weighter, 'update_bandwidth') and
            abs(old_kde_bandwidth - new_config.kde_bandwidth) > 1e-6):
            logger.info("Updating KDE bandwidth due to config change")
            getattr(self.crime_weighter, 'update_bandwidth')(new_config.kde_bandwidth)
    ...
    def get_crime_statistics(self) -> Dict[str, Any]:
        ...
        if self.crime_weighter:
            ...
            if hasattr(self.crime_weighter, 'get_kde_parameters'):
                kde_params = getattr(self.crime_weighter, 'get_kde_parameters')()
                stats['kde_parameters'] = kde_params
            elif hasattr(self.crime_weighter, 'get_proximity_parameters'):
                proximity_params = getattr(self.crime_weighter, 'get_proximity_parameters')()
                stats['proximity_parameters'] = proximity_params
